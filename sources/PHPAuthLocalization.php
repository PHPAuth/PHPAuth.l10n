...
class PHPAuthLocalization implements PHPAuthLocalizationInterface
{
    /**
     * @var string
     */
     ...
     * @var bool
     */
    private $use_fallback_dictionary;

    /**
     * Set language code
     *
     * @param string $language
     * @param bool $use_fallback_dictionary
     */
    public function __construct(string $language = 'en_GB', bool $use_fallback_dictionary = false)
    {
        $this->current_language = $language;
        $this->use_fallback_dictionary = $use_fallback_dictionary;
    }

    /*public function __invoke(string $language = 'en_GB')
    {
        $this->current_language = $language;
        return $this->use();
    }*/
    ...
    public function use():array
    {
        ...
        } elseif ($this->use_fallback_dictionary) {
            $dictionary = $this->getForgottenDictionary();
        }
    ...
    {
        $sth = $pdo->query("SELECT `translation_key`, `{$this->current_language}` as `lang` FROM {$sql_table} ");

        if ($sth === false) {
            return
                $this->use_fallback_dictionary
                ? $this->getForgottenDictionary()
                : [];
        }
